...
import cn.demo.dao.ShopOrderMapper;
import cn.demo.dao.TransactionLogDao;
...
public class OrderServiceImpl {
    ...
    public long submitOrder(ShopOrder shopOrder) {
        ...
        try {
            ...
            //发送普通消息  2次  造成消息的重复 模拟重复订单，测幂等方案
            //MQShopOrder(shopOrder);

            //上面生成订单，但未支付。下面 主要作用，超时未支付的，要提前释放
            //发送延时消息，MQDelayOrder配合事务消息，就是创单+支付流程。
    ...
    //超时订单处理：查看订单是否是终态，如果不是，那就要把订单置为无效
